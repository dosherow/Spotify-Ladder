...
def electronic_daily():
    def pull_playlist():

        # write playlist charts to each sheet in master
        writer_electronic = pd.ExcelWriter("electronicFull" + now_file + ".xlsx", engine="xlsxwriter")
        ...
        for electronic_Id in electronic_Ids:
            playlist_Obj = sp.playlist(electronic_Id[1])
            playlist_followers = playlist_Obj["followers"]["total"]
            spot_Playlists = sp.playlist_tracks(electronic_Id[1])
            ...
            df.to_excel(writer_electronic, sheet_name=electronic_Id[0], index=False, header=True)
    ...
    def pivot():
        ...
        electronic_df = pd.concat(
            pd.read_excel("electronicFull" + now_file + ".xlsx", sheet_name=None), ignore_index=True
        )

        # replacing blank release dates with null value
        electronic_df["Release Date"].replace("", np.nan, inplace=True)
        # dropping null value rows for release date for bad metadata
        electronic_df.dropna(subset=["Release Date"], inplace=True)
        # making release date column a datetime value
        electronic_df["Release Date"] = pd.to_datetime(electronic_df["Release Date"])
        ...
        electronic_pivot["Total Playlists"] = electronic_pivot.sum(axis=1)

        # putting total playlists column at front
        cols_full = [electronic_pivot.columns[-1]] + [
            cols_full for cols_full in electronic_pivot if cols_full != electronic_pivot.columns[-1]
        ]
        ...
        range = electronic_pivot["Release Date"] > cut_off_date
        ...
        electronic_sorted_pivot = electronic_pivot.sort_values(by="Total Playlists", ascending=False)

        ### CREATE NEW CSV HERE ALL PLAYLIST TRACKS WITHIN DATE CUTOFF ###

        with open("electronicPivotFull" + now_file + ".csv", "a") as f:
