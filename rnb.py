...
def rnb_daily():
    def pull_playlist():
        ...
        writer_rnb = pd.ExcelWriter("rnbFull" + now_file + ".xlsx", engine="xlsxwriter")
        ...
        for rnb_Id in rnb_Ids:
            playlist_Obj = sp.playlist(rnb_Id[1])
            playlist_followers = playlist_Obj["followers"]["total"]
            spot_Playlists = sp.playlist_tracks(rnb_Id[1])
            ...
            df.to_excel(writer_rnb, sheet_name=rnb_Id[0], index=False, header=True)
    ...
    def pivot():
        ...
        rnb_df = pd.concat(
            pd.read_excel("rnbFull" + now_file + ".xlsx", sheet_name=None), ignore_index=True
        )

        # replacing blank release dates with null value
        rnb_df["Release Date"].replace("", np.nan, inplace=True)
        # dropping null value rows for release date for bad metadata
        rnb_df.dropna(subset=["Release Date"], inplace=True)
        # making release date column a datetime value
        rnb_df["Release Date"] = pd.to_datetime(rnb_df["Release Date"])
        ...
        rnb_pivot["Total Playlists"] = rnb_pivot.sum(axis=1)

        # putting total playlists column at front
        cols_full = [rnb_pivot.columns[-1]] + [
            cols_full for cols_full in rnb_pivot if cols_full != rnb_pivot.columns[-1]
        ]
        rnb_pivot = rnb_pivot[cols_full]

        rnb_pivot.reset_index(
            level=[
                "Artist",
                "Date",
                "Track Popularity",
        ...
        range = rnb_pivot["Release Date"] > cut_off_date
        # # putting the date range / cutoff together with the dataframe
        rnb_pivot = rnb_pivot.loc[range]

        # sorting table by total playlists track found in, from greatest to least
        rnb_sorted_pivot = rnb_pivot.sort_values(by="Total Playlists", ascending=False)

        ### CREATE NEW CSV HERE ALL PLAYLIST TRACKS WITHIN DATE CUTOFF ###

        with open("rnbPivotFull" + now_file + ".csv", "a") as f:
            rnb_sorted_pivot.to_csv(f, header=True, index=True)
