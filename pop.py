...
def pop_daily():
    def pull_playlist():
        ...
        writer_pop = pd.ExcelWriter("popFull" + now_file + ".xlsx", engine="xlsxwriter")
        ...
        for pop_Id in pop_Ids:
            playlist_Obj = sp.playlist(pop_Id[1])
            playlist_followers = playlist_Obj["followers"]["total"]
            spot_Playlists = sp.playlist_tracks(pop_Id[1])
            ...
            df.to_excel(writer_pop, sheet_name=pop_Id[0], index=False, header=True)
    ...
    def pivot():
        ...
        pop_df = pd.concat(
            pd.read_excel("popFull" + now_file + ".xlsx", sheet_name=None), ignore_index=True
        )

        # replacing blank release dates with null value
        pop_df["Release Date"].replace("", np.nan, inplace=True)
        # dropping null value rows for release date for bad metadata
        pop_df.dropna(subset=["Release Date"], inplace=True)
        # making release date column a datetime value
        pop_df["Release Date"] = pd.to_datetime(pop_df["Release Date"])
        ...
        pop_pivot["Total Playlists"] = pop_pivot.sum(axis=1)

        # putting total playlists column at front
        cols_full = [pop_pivot.columns[-1]] + [
            cols_full for cols_full in pop_pivot if cols_full != pop_pivot.columns[-1]
        ]
        pop_pivot = pop_pivot[cols_full]

        pop_pivot.reset_index(
            level=[
                "Artist",
                "Date",
                "Track Popularity",
        ...
        range = pop_pivot["Release Date"] > cut_off_date
        # # putting the date range / cutoff together with the dataframe
        pop_pivot = pop_pivot.loc[range]

        # sorting table by total playlists track found in, from greatest to least
        pop_sorted_pivot = pop_pivot.sort_values(by="Total Playlists", ascending=False)

        ### CREATE NEW CSV HERE ALL PLAYLIST TRACKS WITHIN DATE CUTOFF ###

        with open("popPivotFull" + now_file + ".csv", "a") as f:
            pop_sorted_pivot.to_csv(f, header=True, index=True)
