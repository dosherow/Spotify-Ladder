...
def rap_daily():
    def pull_playlist():
        ...
        writer_rap = pd.ExcelWriter("rapFull" + now_file + ".xlsx", engine="xlsxwriter")
        ...
        for rap_Id in rap_Ids:
            playlist_Obj = sp.playlist(rap_Id[1])
            playlist_followers = playlist_Obj["followers"]["total"]
            spot_Playlists = sp.playlist_tracks(rap_Id[1])
            ...
            df.to_excel(writer_rap, sheet_name=rap_Id[0], index=False, header=True)
    ...
    def pivot():
        ...
        rap_df = pd.concat(
            pd.read_excel("rapFull" + now_file + ".xlsx", sheet_name=None), ignore_index=True
        )

        # replacing blank release dates with null value
        rap_df["Release Date"].replace("", np.nan, inplace=True)
        # dropping null value rows for release date for bad metadata
        rap_df.dropna(subset=["Release Date"], inplace=True)
        # making release date column a datetime value
        rap_df["Release Date"] = pd.to_datetime(rap_df["Release Date"])
        ...
        rap_pivot["Total Playlists"] = rap_pivot.sum(axis=1)

        # putting total playlists column at front
        cols_full = [rap_pivot.columns[-1]] + [
            cols_full for cols_full in rap_pivot if cols_full != rap_pivot.columns[-1]
        ]
        rap_pivot = rap_pivot[cols_full]

        rap_pivot.reset_index(
            level=[
                "Artist",
                "Date",
                "Track Popularity",
        ...
        range = rap_pivot["Release Date"] > cut_off_date
        # # putting the date range / cutoff together with the dataframe
        rap_pivot = rap_pivot.loc[range]

        # sorting table by total playlists track found in, from greatest to least
        rap_sorted_pivot = rap_pivot.sort_values(by="Total Playlists", ascending=False)

        ### CREATE NEW CSV HERE ALL PLAYLIST TRACKS WITHIN DATE CUTOFF ###

        with open("rapPivotFull" + now_file + ".csv", "a") as f:
            rap_sorted_pivot.to_csv(f, header=True, index=True)
