...
def country_daily():
    def pull_playlist():
        ...
        writer_country = pd.ExcelWriter("countryFull" + now_file + ".xlsx", engine="xlsxwriter")
        ...
        for country_Id in country_Ids:
            playlist_Obj = sp.playlist(country_Id[1])
            playlist_followers = playlist_Obj["followers"]["total"]
            spot_Playlists = sp.playlist_tracks(country_Id[1])
            ...
            df.to_excel(writer_country, sheet_name=country_Id[0], index=False, header=True)
    ...
    def pivot():
        ...
        country_df = pd.concat(
            pd.read_excel("countryFull" + now_file + ".xlsx", sheet_name=None), ignore_index=True
        )

        # replacing blank release dates with null value
        country_df["Release Date"].replace("", np.nan, inplace=True)
        # dropping null value rows for release date for bad metadata
        country_df.dropna(subset=["Release Date"], inplace=True)
        # making release date column a datetime value
        country_df["Release Date"] = pd.to_datetime(country_df["Release Date"])
        ...
        country_pivot["Total Playlists"] = country_pivot.sum(axis=1)

        # putting total playlists column at front
        cols_full = [country_pivot.columns[-1]] + [
            cols_full for cols_full in country_pivot if cols_full != country_pivot.columns[-1]
        ]
        country_pivot = country_pivot[cols_full]

        country_pivot.reset_index(
            level=[
                "Artist",
                "Date",
                "Track Popularity",
        ...
        range = country_pivot["Release Date"] > cut_off_date
        # # putting the date range / cutoff together with the dataframe
        country_pivot = country_pivot.loc[range]

        # sorting table by total playlists track found in, from greatest to least
        country_sorted_pivot = country_pivot.sort_values(by="Total Playlists", ascending=False)

        ### CREATE NEW CSV HERE ALL PLAYLIST TRACKS WITHIN DATE CUTOFF ###

        with open("countryPivotFull" + now_file + ".csv", "a") as f:
            country_sorted_pivot.to_csv(f, header=True, index=True)
